...
# 📝 概要
本プロジェクトは、近年蔓延している「クリーナーアプリ」を装った広告詐欺や、過剰な権限要求を行う不審なアプリから一般ユーザーを保護するために開発されました。Accessibility Serviceを活用してPlayストアの情報をリアルタイムに抽出し、バックエンドのMLモデル（NLP + 特徴量分析）及びLLMによってリスクをスコアリングします。
BERT（1次判定）: アプリの説明文と権限情報を高速に計算。
Gemini（2次判定）: プロンプトで「なぜ怪しいのか」も含めて詳細解析。

判定の分岐:
スコアが 0.55以上 → 即座に「詐欺（高）」
スコアが 0.45未満 → 即座に「安全」
スコアが 0.45〜0.55 → **Gemini（2次判定）**へパス

Gemini（2次判定）: 
プロンプトで「なぜ怪しいのか」も含めて詳細解析。

コスト削減: ほとんどの「明らかに安全」「明らかに怪しい」アプリは、無料に近いBERTだけで処理されます。
根拠の提示: Geminiを使うことで、Androidアプリ側に「なぜ怪しいのか」という具体的な解説を表示できるようになります。
精度の補完: BERTはキーワードに反応しがちですが、Geminiは「文章の不自然な日本語」や「文脈の矛盾」を見抜けます
